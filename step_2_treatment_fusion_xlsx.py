...
def traitement_fusion_xlsx_c9_c13(path_folder_excel, path_folder_fusion_excel):
    ...
    for date in dates_fichiers:
        ...
        if len(list_files_with_date_selected) == 2:
            ...
            if len(list_file_with_date_selected) == 0:
                ...
                if "C9" in list_files_with_date_selected[0]:
                    df_c9 = pd.read_excel(os.path.join(path_folder_excel, list_files_with_date_selected[0]), dtype={"code_postal": str})
                    df_c13 = pd.read_excel(os.path.join(path_folder_excel, list_files_with_date_selected[1]), dtype={"code_postal": str})
                else:
                    df_c9 = pd.read_excel(os.path.join(path_folder_excel, list_files_with_date_selected[1]), dtype={"code_postal": str})
                    df_c13 = pd.read_excel(os.path.join(path_folder_excel, list_files_with_date_selected[0]), dtype={"code_postal": str})
                ...
                code_pr_c9_sans_c13 = code_pr_c9.difference(code_pr_c13)
                code_pr_c13_sans_c9 = code_pr_c13.difference(code_pr_c9)
                ...
                df_c9_sans_c13 = df_c9[df_c9["code_point_relais"].isin(list(code_pr_c9_sans_c13))]
                df_c13_sans_c9 = df_c13[df_c13["code_point_relais"].isin(list(code_pr_c13_sans_c9))]
                df = pd.concat([df_c9_c13, df_c9_sans_c13, df_c13_sans_c9], sort=True)
                df = df[col]
...
if __name__ == "__main__":
    traitement_fusion_xlsx_c9_c13(os.path.join(PATH_ONEDRIVE, FOLDER_C9_C13_EXCEL), os.path.join(PATH_ONEDRIVE, FOLDER_C9_C13_FUSION_EXCEL))
